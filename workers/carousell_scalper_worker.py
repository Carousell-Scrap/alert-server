...
@celery.task()
def scrape_carousell_with_params(query, alert_id, chat_id, from_range=None,
                                 to_range=None, is_first_time=False):
    ...
    try:
        ...
        for alert in alerts_to_scrape:
            ...
            scrape_carousell_with_params.delay(
                alert.query, alert.id, user_id, alert.from_price,
                alert.to_price, alert.is_first_scrape)
...
async def send_messages(chat_id: str, messages: list, query: str, from_range: str,
                        to_range: str):
...
def create_listing_to_db(items: list, alert_id: str):
    ...
    for item in items:
        listing = client.collection('listings').get_list(1, 1, query_params={
