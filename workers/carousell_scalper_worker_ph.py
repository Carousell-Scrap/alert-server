...
def scrape_carousell_with_params(
    query, alert_id, chat_id, from_range=None, to_range=None, is_first_time=False
):
    ...
    try:
        ...
        items = scrape_page(soup, alert_id)

        items_created, messages = create_listing_to_db(items, alert_id)

        print("sending messages...")
        if is_first_time:
            messages = [
                f"Alert ran for the first time and found {items_created} new listings! \
                    Subsequent alerts will only send you new listings.\n"
            ]  # noqa: E501
            asyncio.run(send_messages(chat_id, messages, query, from_range, to_range))
        else:
            if items_created > 0:
                asyncio.run(
                    send_messages(chat_id, messages, query, from_range, to_range)
                )
...
def scrape_page(soup: BeautifulSoup, alert_id: str):
...
async def send_messages(
    chat_id: str, messages: list, query: str, from_range: str, to_range: str
):
...
def create_listing_to_db(items: list, alert_id: str):
