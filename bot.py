...
SUBSCRIBE_TO_ALERT,\
    ...
    SUBSCRIBE_TO_ALERT_CONFIRMATION = range(5)
...
def create_alert(query, from_price, to_price, created_by):
    ...
    nextTimeToRun = utils.get_alert_next_time_to_run(minutes=1)
...
async def use_code_process(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        client.collection('codes').update(codes.items[0].id, {
            'subscribed_by': chat_id
        })
    ...
    if is_error:
        await update.message.reply_text(f'{message} Please try again from the start!',
                                        reply_markup=ReplyKeyboardRemove())
    else:
        await update.message.reply_html(
            f'Code is valid! You have successfully redeemed <b> {message} </b> new \
                alerts your code! 🎉🎉', reply_markup=ReplyKeyboardRemove()
        )
...
async def subscribe_to_alert(update: Update, context: ContextTypes.DEFAULT_TYPE) \
    ...
    if alerts_left == 0:
        await update.message.reply_text('You have no alerts left! Please use a code \
            to get more alerts!')
        return ConversationHandler.END
...
async def subscribe_to_alert_query(update: Update, context: ContextTypes.DEFAULT_TYPE)\
        -> None:
...
Now, please enter the <b>minimum price</b> you want to be alerted at. Please send 0 if you do not want to set a minimum price.
...
async def subscribe_to_alert_from_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
Now, please enter the <b>maximum price</b> you want to be alerted at. Please send 0 if you do not want to set a maximum price.
    ''')

    return SUBSCRIBE_TO_ALERT_TO_PRICE


async def subscribe_to_alert_to_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def subscribe_to_alert_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    print('subscribe_to_alert_confirmation')
    try:
        confirmation = update.message.text
        if confirmation.lower() != 'confirm':
            await update.message.reply_text('Please enter confirm to confirm your\
                subscription,\ otherwise type /cancel to \
                    cancel and restart this process.')
            return SUBSCRIBE_TO_ALERT_CONFIRMATION

        query = context.user_data["query"]
        from_price = None if context.user_data['from_price'] == '0' else float(
            context.user_data['from_price'])
        to_price = None if context.user_data['to_price'] == '0' else float(
            context.user_data['to_price'])
...
You can view your alerts by typing /my_alerts.
        ...
        await update.message.reply_text(f'There was an error creating your alert.\
...
async def check_alerts_left(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_error:
        await update.message.reply_text(f'{message} Please try again!', reply_markup=ReplyKeyboardRemove())
    else:
        await update.message.reply_html(
...
Thank you for your interest in using this bot, we are currently in beta and are only giving out codes to a selected few. If you are interested, please fill in this form and we will get back to you as soon as possible!
    ''',
        reply_markup=ForceReply(selective=True),
    )
...
if __name__ == "__main__":
    ...
    use_code_handler = ConversationHandler(
        ...
        states={
            PROCESS_CODE: [MessageHandler(filters.TEXT & ~filters.COMMAND,
                                          use_code_process)],
        },
    ...
    subscribe_to_alert_handler = ConversationHandler(
        entry_points=[CommandHandler(
            "subscribe_alert", subscribe_to_alert)],
        states={
            SUBSCRIBE_TO_ALERT_QUERY: [MessageHandler
                                       (filters.TEXT & ~filters.COMMAND,
                                        subscribe_to_alert_query)],
            SUBSCRIBE_TO_ALERT_FROM_PRICE: [MessageHandler
                                            (filters.TEXT & ~filters.COMMAND,
                                             subscribe_to_alert_from_price)],
            SUBSCRIBE_TO_ALERT_TO_PRICE: [MessageHandler
                                          (filters.TEXT & ~filters.COMMAND,
                                           subscribe_to_alert_to_price)],
            SUBSCRIBE_TO_ALERT_CONFIRMATION: [MessageHandler
                                              (filters.TEXT & ~filters.COMMAND,
